...
struct ScoringListener : Catch::TestEventListenerBase
{
    ...
    sectionEnded (Catch::SectionStats const& sectionStats) override
    {
        ...
        const auto begin_idx =
          sectionStats.sectionInfo.name.find_last_of ('[') + 1;
        const auto end_idx = sectionStats.sectionInfo.name.size () - 1;
        const double score = stod (sectionStats.sectionInfo.name.substr (
        ...
        if (sectionStats.assertions.total () > 0)
        {
            if (!sectionStats.assertions.allPassed ())
            {
                ...
                stream << "- " << fixed << setprecision (3) << point_value_num
                       << " points:   " << sectionStats.sectionInfo.name;
...
TEST_CASE ("insert (12)")
{
    ...
    SECTION ("insert - return value 2")
    {
        ...
        SECTION ("insert - return value - on successful insert 1")
        {
            ...
            SECTION (
              "insert - return value - on successful insert - boolean value "
              "[0.25]")
            {
                REQUIRE (ret.second);
            }
...
TEST_CASE ("erase (18)")
{
    ...
    SECTION ("erase - second largest/second smallest values 1")
    {
        ...
        // Dependencies: size
        SECTION (
          "erase - second largest/second smallest values - size of tree [0.05]")
        {
            REQUIRE (tree.size () == 3);
        }
        ...
        // Dependencies: end
        SECTION (
          "erase - second largest/second smallest values - head links [0.20]")
        {
    ...
    SECTION ("erase - return value 2")
    {
        ...
        {
            SECTION (
              "erase - return value - on unsuccessful erase - on empty tree "
              "[0.50]")
            {
            ...
            // Dependencies: insert
            SECTION (
              "erase - return value - on unsuccessful erase - with value not "
              "in tree [0.50]")
            {
                tree_insert (tree, {4, 2, 7, 6});
        ...
        {
            SECTION (
              "erase - return value - on successful erase - on last value in "
              "tree [0.25]")
            {
                tree.insert (7);
            ...
            SECTION (
              "erase - return value - on successful erase - on value other "
              "than last in tree [0.75]")
            {
                tree_insert (tree, {2, 3, 1});
...
TEST_CASE ("print level order (12)")
{
    ...
    {
        tree_insert (tree, {49, 14,  12, 17, 65,  223, 4,   7,   6,   11, 14,
                            21, 15,  1,  2,  3,   8,   10,  44,  45,  46, 39,
                            81, 100, 69, 73, 19,  18,  31,  32,  77,  76, 78,
                            45, 47,  13, 64, 63,  66,  250, 101, 102, 16, 50,
                            51, 52,  53, 99, 104, 300, 999, 5});

        REQUIRE_LEVEL_ORDER (tree,
                             "[ 49 | 14 65 | 12 17 64 223 | 4 13 15 21 63 - 81 "
                             "250 | 1 7 - - - 16 19 44 50 - - - 69 100 "
                             "- 300 | - 2 6 11 - - - - - - - - 18 - 39 45 - 51 "
                             "- - - - - - 66 73 99 101 - - - 999 | - - "
                             "- 3 5 - 8 - - - - - - - - - - - - - - - - - - - "
                             "- - 31 - - 46 - - - 52 - - - - - - - - - -"
                             " - - - - - 77 - - - 102 - - - - - - - - | - - - "
...
TEST_CASE ("node explicit constructor (3)")
{
    ...
    SECTION (
      "node explicit constructor - with links to nodes created with default "
      "constructor [1.00]")
    {
        Node<int> node_a (1);
        Node<int> node_b;
    ...
    SECTION (
      "node explicit constructor - with links to nodes created with explicit "
      "constructor [1.00]")
    {
        ...
        REQUIRE_ALL_EQUAL<Node<int>*> ({node_a.left, node_a.right,
                                        node_a.parent, node_b.left,
                                        node_b.parent, node_c.right, nullptr});
        REQUIRE_ALL_EQUAL<Node<int>*> (
          {&node_a, node_b.right, node_c.parent, node_d.left});
    ...
    {
        Node<string> node ("print level order is fun", nullptr, nullptr,
                           nullptr);
...
TEST_CASE ("iterator post-increment (2)")
{
    ...
    SECTION (
      "iterator post-increment - return value from incrementation [1.00]")
    {
...
TEST_CASE ("iterator post-decrement (2)")
{
    ...
    SECTION (
      "iterator post-decrement - return value from decrementation [1.00]")
    {
        SearchTree<int>::iterator expected = iter, actual = iter;
        actual = iter--;
...
TEST_CASE ("iterator increment (10)")
{
    ...
    {
        SECTION (
          "iterator increment - on node that has a right child - with no "
          "subtree [1.00]")
        {
        ...
        SECTION (
          "iterator increment - on node that has a right child - with a "
          "subtree [1.00]")
        {
    ...
    SECTION (
      "iterator increment - on node that is the left child of the tree root "
      "when it is the maximum value node in tree [1.00]")
    {
        tree.insert (6);
        iter = (tree.insert (4)).first;
...
TEST_CASE ("iterator decrement (10)")
{
    ...
    {
        SECTION (
          "iterator decrement - on node that has a left child - with no "
          "subtree [1.00]")
        {
    ...
    SECTION (
      "iterator decrement - on node that is the right child of the tree root "
      "when it is the minimum value node in tree [1.00]")
    {
        tree.insert (6);
        iter = (tree.insert (8)).first;
...
REQUIRE_TREE (const SearchTree<T>& tree, string treeString)
{
    ...
    output << "]";
    // std::cout << "printInOrder = " << output.str() << "\n";
...
REQUIRE_LEVEL_ORDER (const SearchTree<T>& tree, string answer)
{
    ...
    out_no_space.erase (
      remove (out_no_space.begin (), out_no_space.end (), ' '),
      out_no_space.end ());
    out_no_dash.erase (remove (out_no_dash.begin (), out_no_dash.end (), '-'),
                       out_no_dash.end ());

    answer_no_space.erase (
      remove (answer_no_space.begin (), answer_no_space.end (), ' '),
      answer_no_space.end ());
    answer_no_dash.erase (
      remove (answer_no_dash.begin (), answer_no_dash.end (), '-'),
      answer_no_dash.end ());
